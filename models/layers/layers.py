...
class GATLayer(nn.Module):
    def __init__(self, c_in, c_out, num_heads=1, concat_heads=True, alpha=0.2):
        ...
        inputs:
            ...
            num_head: number of attention heads. here we only use 1
            concat_heads: Whether to concat attention heads
        ...
        if self.concat_heads:
            assert c_out % num_heads == 0, 'Number of outupt features must be multiple of the count of heads'
    ...
    def forward(self, node_feats, adj_matrix, return_attn_probs=True):
        '''
        Inputs:
            node_feats: input features of nodes [batch-size, num_nodes, c_in]
            adj_matrix: adjacency matrix [batch-size, num_nodes, num_nodes] with self connections
            return_attn_probs: If True, attention weights will be returned
        '''
        ...
        # concatenate the node feature of all the nodes that are connected
        edges = adj_matrix.nonzero(as_tuple=False)  # return edges
        ...
        a_input = torch.cat([
            ...
            torch.index_select(input=node_feats_flat, index=edge_indices_col, dim=0)], dim=-1)
...
class NeighborMultiHeadAttention(nn.Module):
    ...
    def _masked_softmax(self, attend_logits, mask_attend, dim=-1):
        ...
        attend_logits = torch.where(mask_attend, attend_logits, torch.tensor(negative_inf).to(device))
        attend = F.softmax(attend_logits, dim)
        attend = mask_attend * attend
        return attend

    def forward(self, h_V, h_E, mask_attend=None, return_attn_probs=True):
        ...
        Returns:
            h_V_t: node update after neighbor attention [batch-size, num_nodes, n_features]
        '''
        # dimensions
        ...
        if return_attn_probs:
            return h_V_update, attend.mean(dim=2)
        else:
            return h_V_update
...
class GraphConvLayer(nn.Module):
    ...
    def forward(self, atom_emb, nbr_emb, nbr_adj_list):
        ...
        atom_nbr_emb = atom_emb[torch.arange(B).unsqueeze(-1), nbr_adj_list.to(torch.long).view(B, -1)].view(B, N, M,
                                                                                                             self.h_a).to(
            device)
        ...
        total_nbr_emb = torch.cat([atom_emb.unsqueeze(2).expand(B, N, M, self.h_a), atom_nbr_emb, nbr_emb], dim=-1).to(
            torch.float32)
        ...
        # Sigmoid function  for edge-gating mechanism
...
class ContinuousFilterConv(nn.Module):
    ...
    def __init__(self, n_gaussians, n_filters, activation=nn.Tanh(), normalization_layer=None):
        ...
        nn.init.xavier_uniform_(self.nbr_filter.data, gain=1.414)
    ...
    def forward(self, features, rbf_expansion, neighbor_list):
        ...
        # [B, N, M, n_features]
        # nbr_filter = self.nbr_filter(conv_features).view([n_batch, n_atoms, 1, n_neighbors])
        ...
        nbr_filter = nbr_filter.view([n_batch, n_atoms, n_neighbors])
        # [B, N, M]
        ...
        ####### later using attention on the connectivities so we need this now #########
        # aggregated_features = conv_features
...
class InteractionBlock(nn.Module):
    ...
    def __init__(self, n_inputs, n_gaussians, n_filters, activation=nn.Tanh(), normalization_layer=None):
        ...
        # self.attn = AttnBlock(n_filters, n_filters)

        # look here if adding layers here will give any problems
    ...
    def forward(self, features, rbf_expansion, neighbor_list):
        ...
        conv_output, attn = self.cfconv(init_feature_output.to(torch.float32), rbf_expansion.to(torch.float32),
                                        neighbor_list)
        # adj = self.convert_adj(neighbor_list)
        # conv_output_t, attn = self.attn(conv_output, neighbor_list)
